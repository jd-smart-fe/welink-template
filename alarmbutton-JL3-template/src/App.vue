<template>
  <div id="app">
    ...
    <!--错误提示条-->
    <div v-show="alarm_ece"  class="errOth" ><div class="icon_error"></div><div class="text_error">电池电量不足，请更换电池！</div></div>
    <!--title-->
  ...
  methods: {
    ...
    singenals(arrObject) {
      ...
      for (let i = 0; i < arrObject.length; i += 1) {
        if (arrObject[i].name === 'battery_voltage') { // 低电压
        ...
        if (arrObject[i].name === 'trigger') {
          if (arrObject[i].data === 'click') {
            if (!self.today_show) {
              self.chageSiganlData();
            }
          }
        }
        if (arrObject[i].name === 'info') {
          if (arrObject[i].data === 'low_voltage') {
            self.alarm_ece = true;
            if (!self.today_show) {
              self.chageSiganlData();
            }
    ...
    getsucForSnapshot(data) { // 快照返回成功
      ...
      if (data.result.errorMsg === 'com.joylink.ok') {
        for (let i = 0; i < arrObject.length; i += 1) {
          if (arrObject[i].name === 'battery_voltage') { // 电压低
            if (arrObject[i].data === 'low') {
              self.alarm_ece = true;
            } else {
              self.alarm_ece = false;
